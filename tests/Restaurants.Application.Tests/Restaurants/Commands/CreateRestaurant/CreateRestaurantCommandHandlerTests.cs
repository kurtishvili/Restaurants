...
public class CreateRestaurantCommandHandlerTests
{
    ...
    public async Task Handle_ForValidCommand_ReturnCreatedRestaurantid()
    {
        ...
        restaurantRepositoryMock.Setup(repo => repo.Create(It.IsAny<Restaurant>(), CancellationToken.None)).ReturnsAsync(1);

        var userContextMock = new Mock<IUserContext>();
        var currnetUser = new CurrentUser("owner_id", "test@test.com", [], null, null);

        userContextMock.Setup(u => u.GetCurrentUser()).Returns(currnetUser);
        ...
        restaurantRepositoryMock.Verify(r => r.Create(restaurant, CancellationToken.None), Times.Once());
    }
}
